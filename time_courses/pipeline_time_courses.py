...
def extract_intensity_and_shift_data(dataframe, selected_field='spot_int_ch_0', 
    ...
    if intensity_array_shifted.shape[0] == 0:
        if verbose:
                print("WARNING: No trajectories survived quality filtering")
        return np.array([]).reshape(0, maximum_columns)
...
def extract_dual_channel_data(dataframe, primary_field, secondary_field, 
    ...
    if primary_filtered.shape[0] == 0:
        print("    ERROR: No trajectories passed dual-channel quality filter")
        return None, None
    ...
    if use_forward_fill:
        ...
        if verbose:
            print("    Applied forward-fill to handle NaNs")
    ...
    if primary_shifted.shape[0] == 0:
        if verbose:
            print("    ERROR: No trajectories passed final quality check")
        return None, None
...
def compute_autocorrelation_for_dataset(
    ...
    if simulation_mode:
        ...
        if verbose:
            print("SIMULATION MODE: Using provided SSA data")
    ...
    for tracking_file in tracking_files:
        try:
            ...
            if primary_array is not None and secondary_array is not None:
                ...
                if verbose:
                    print("  SKIP: No valid synchronized trajectories")
...
def analyze_crosscorr(
    ...
    if min_max_normalize:    
        ...
        if corr_max > corr_min:
            ...
            print("Min-max normalization applied to entire array using window range")
        else:
            # All values in window are the same
            corr_interp = corr_interp - corr_min
            print("Warning: Constant correlation values in window, centering at zero")
...
def analyze_minima_vs_secondary(
    ...
    for i in range(n_traces):
        ...
        # Keep detection unsmoothed to preserve sharp minima features.
...
def plot_aligned_with_shared_normalization(
    ...
    style_legend(ax.legend(loc='upper left', fontsize=11))
    ...
    if save_plot and results_folder is not None:
        ...
        plt.savefig(results_folder / f'{base_name}.png', dpi=300, bbox_inches='tight', facecolor='white')
        plt.savefig(results_folder / f'{base_name}.svg', bbox_inches='tight', facecolor='white')
    ...
    if p_delay < 0.05 and ci_lower > 0:
        print("CONCLUSION: Significant POSITIVE delay detected")
        print(f"  {secondary_label} lags behind {primary_label} by ~{delay * step_size_in_sec:.1f} seconds")
    elif p_delay < 0.05 and ci_upper < 0:
        print("CONCLUSION: Significant NEGATIVE delay detected")
        print(f"  {secondary_label} leads {primary_label} by ~{-delay * step_size_in_sec:.1f} seconds")
    elif ci_lower <= 0 <= ci_upper:
        print("CONCLUSION: No conclusive delay")
        print("  Confidence interval includes zero - high uncertainty")
    else:
        print("CONCLUSION: Ambiguous result")
    print(f"{'='*70}\n")
...
def analyze_dual_channel_time_courses(
    ...
    if simulation_mode:
        ...
        if SSA_data_1.shape != SSA_data_2.shape:
            raise ValueError("SSA_data_1 and SSA_data_2 must have same shape")
    ...
    if trajectory_colors is None:
        trajectory_colors = [CHANNEL_GREEN, CHANNEL_MAGENTA]
    ...
    if verbose:
        print(f"{'='*60}")
        print("Analyzing Dual-Channel Time Courses")
        print(f"Data source: {'SIMULATION' if simulation_mode else 'EXPERIMENTAL'}")
...
def plot_dual_signal_trajectories(
    ...
    if colors is None:
        colors = [CHANNEL_GREEN, CHANNEL_MAGENTA]
    if labels is None:
    ...
    ax.plot(time_array[:len(primary_trace)], primary_trace, 
            color=colors[0], linewidth=1.6, label=labels[0], alpha=0.95)
    ax.plot(time_array[:len(secondary_trace)], secondary_trace, 
            color=colors[1], linewidth=1.6, label=labels[1], alpha=0.95)
    
    ax.set_xlabel('Time (s)', fontsize=10)
    ax.set_ylabel('Normalized Intensity' if normalize else 'Intensity (a.u.)', fontsize=10)
    ax.set_title(f'Trajectory {trajectory_index}', fontsize=11)
    style_axes(ax, grid=False, spine_width=1.2, tick_size=9, label_size=10, title_size=11)
    style_legend(ax.legend(loc='upper right', fontsize=9))
    ...
    if save_plot and results_folder is not None:
        ...
        fig.savefig(results_folder / f'{base_name}.png', dpi=300, bbox_inches='tight', facecolor='white')
        fig.savefig(results_folder / f'{base_name}.svg', bbox_inches='tight', facecolor='white')
        if verbose:
...
def plot_dual_channel_kymograph(
    ...
    if channel_colors is None:
        try:
            ...
            primary_color = CHANNEL_GREEN
            secondary_color = CHANNEL_MAGENTA
    else:
    ...
    if normalize is None:
        ...
        if verbose:
            ...
            print("  Scaling raw data to [0, 1] using global percentiles (robust to outliers)")
        ...
        if all_finite_primary.size > 0 and all_finite_secondary.size > 0:
            ...
            if verbose:
                print("  WARNING: No finite data found, using zeros")
    ...
    ax.set_xlim(0, W)
    ax.set_ylim(H, 0)
    style_axes(ax, grid=False, spine_width=1.5, tick_size=11, label_size=12, title_size=13)
    ...
    if save_path:
        ...
        fig.savefig(f'{base_path}.png', bbox_inches="tight", dpi=dpi, facecolor="white")
