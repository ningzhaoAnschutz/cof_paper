...
(C) Fitted parameters: $\kfold = 0.014$ s$^{-1}$ ($\taufold = 71$ s), $f_{\text{base}} = 0.37$, $f_{\text{gain}} = 0.14$.
(D) Translation timeline showing the time windows for folding for each GFP domain.}
...
The Two-Pool model explains the saturation behavior by proposing that only $\sim$37\% of nascent chains are initially competent for co-translational folding, with each additional GFP domain increasing this fraction by $\sim$14\% (avidity effect). The fitted folding rate $\kfold = 0.014$ s$^{-1}$ ($\taufold \approx 71$s) is consistent with in vitro GFP folding kinetics. The time constraint hypothesis explains why later GFP domains contribute less to the signal: they have progressively shorter time windows before translation terminates and the polysome disassembles, leaving insufficient time to complete the folding process.
