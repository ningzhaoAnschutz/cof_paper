...
def load_condition_intensities(
    ...
    matching_folders = sorted(
        [
            f
            for f in dataframes_dir.iterdir()
            if f.is_dir()
            and f.name.startswith("results_")
            and folder_substring in f.name
        ],
        key=lambda x: x.name,
    )
        ...
        try:
            ...
            if coloc_filter is not None:
                if "is_colocalized" not in df.columns:
                    if verbose:
                        print(f"  SKIP {cell_name}: " f"no 'is_colocalized' column")
                ...
                traj_coloc = df.groupby(particle_col)["is_colocalized"].any()
            ...
            if verbose:
                print(
                    f"  Loaded {cell_name}: {len(particle_means)} particles, "
    ...
    if verbose:
        print(
            ...
            f"{result['n_particles_total']} particles total"
        )
    return result
...
def plot_per_cell_distributions(
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(4 * ncols, 3.5 * nrows), squeeze=False, facecolor="white"
    )
    ...
    for idx, cell in enumerate(cells):
        ...
        ax.axvline(
            np.median(vals), color=color, linestyle="--", linewidth=1.0, alpha=0.7
        )
    ...
    fig.suptitle(
        f"{cond_name} — ch{channel_index} ({mode_label})",
    ...
    for ext in ("svg", "png"):
        fig.savefig(
            save_dir / f"{cond_name}_per_cell_mode{mode}_ch{channel_index}.{ext}",
            dpi=300,
            bbox_inches="tight",
        )
    plt.show()
...
def plot_pooled_distributions(
    ...
    if xlim is None:
        xlim = _auto_xlim(combined, xlim_percentiles, drop_nonpositive=drop_nonpositive)
    ...
    # Two-sample Kolmogorov-Smirnov test on pooled particle distributions.
    if print_stats and len(all_results) == 2:
        vals_a = _finite_values(
            _get_data_for_mode(all_results[0], mode), drop_nonpositive
        )
        vals_b = _finite_values(
            _get_data_for_mode(all_results[1], mode), drop_nonpositive
        )
        if vals_a.size >= 2 and vals_b.size >= 2:
            ...
            print(
                "    D statistic: max distance between the two CDFs "
                "(0 = identical, 1 = completely separated)"
            )
            print(
                "    p-value: probability of observing D this large "
                "if the distributions were truly identical"
            )
    ...
    ax.set_ylabel(
        "Probability Density", fontsize=_LABEL_SIZE, fontname=_FONT, color="black"
    )
    ...
    for ext in ("svg", "png"):
        fig.savefig(
            save_dir / f"intensity_dist_ch{channel_index}_{mode_label}.{ext}",
            dpi=300,
            bbox_inches="tight",
        )
    plt.show()
...
def plot_cell_summary(
    ...
    ax.set_ylabel(
        f"{y_col} intensity (ch {channel_index})",
